...
class TestLogTracing:
    ...
    rho = game.random_strategy(seed=23, zeros=True)

    hom = sgamesolver.homotopy.LogTracing(game, rho=rho, nu=nu)
    hom_np = sgamesolver.homotopy._logtracing.LogTracing_np(game, rho=rho, nu=nu)
    hom_ct = sgamesolver.homotopy._logtracing.LogTracing_ct(game, rho=rho, nu=nu)
    ...
    def test_solve(self):
        ...
        assert np.max(np.abs(self.hom.H(sol['y']))) < self.hom_np.default_parameters['corrector_tol']
