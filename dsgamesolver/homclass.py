...
class HomCont:
    """Class to perform homotopy continuation to solve a nonlinear system of equations: F(x) = H(x, t_target) = 0.
        ...
        parameters : dict
            ...
            .set_parameters() allows to adjust parameters after construction, again passed as dict or kwargs.
            ...
            H_tol : float
                Convergence criterion used for corrector step: max(H(y_corr)) < H_tol. Defaults to 1e-7.
            corr_steps_max : int
    ...
    @y.setter
    def y(self, value):
        """Set current point. Update flags for derived variables."""
        self._J_needs_update = True
        self._tangent_needs_update = True
        self._cond_needs_update = True
        self._y = value
    ...
    def y_pred(self, value):
        """Set predictor point. Update flags for derived variables."""
        self._J_pred_needs_update = True
        self._Jpinv_needs_update = True
        self._y_pred = value
    ...
    def solve(self):
        ...
        while not self.converged:
            ...
            if self.corrector_success:
                ...
                # J at y_corr was already computed and can be reused:
                self.J = self.J_corr
                ...
                if self.store_path:
                    self.path.update(y=self.y, s=self.s, cond=self.cond,
                                     sign=self.sign, step=self.iteration)
                if self.verbose >= 2:
                    sys.stdout.write(f'\rStep {self.iteration:5d}: t = {self.t:#.4g},  '
            ...
            if self.failed:
                ...
                if self.verbose >= 1:
                    ...
                    sys.stdout.write(f'Step {self.iteration:5d}: Continuation failed. '
                                     f'Total time elapsed:{timedelta(seconds=int(time_sec))} \n')
                    sys.stdout.flush()
    ...
    def correct(self):
        ...
        # After convergence of corrector loop:
        # Check change in augmented determinant: An indicator of potential segment jumping.
        # If change is too large, discard new point and repeat prediction step with shorter stepsize.
        ...
        if det_ratio < self.detJ_change_max or det_ratio > 1/self.detJ_change_max:
            if self.verbose >= 2:
                sys.stdout.write(f'\nStep {self.iteration:5d}: Possible segment jump, discarding predictor. '
                                 f'Augmented determinant ratio: det(J_new)/det(J_old) = {det_ratio:0.2f}\n')
                sys.stdout.flush()
            return
    ...
    def check_convergence(self):
        """Check whether convergence is achieved.
           ...
           2 possible criteria:
               a) t_target is given and finite. Then convergence is achieved if |t_target - t_current| < t_tol.
                  [This function also checks if the corrector step accidentally crossed t_target.
                   This should be rare, due to stepsize control. If so, the current step is discarded,
                   and the algorithm will reduce ds, then return to prediction step.]
               b) t_target is inf. Then convergence is achieved once all variables (besides t)
                  are stable from step to step, and step size is maximal.
    ...
    def check_bifurcation(self):
        ...
        if angle(self.tangent_old, self.tangent) > self.bifurc_angle_min:
            if self.verbose >= 2:
                sys.stdout.write(f'\nStep {self.iteration:5d}: Bifurcation point encountered '
...
class HomPath:
    ...
    def get_step(self, step_no: int):
        """Returns data for step_no if possible.
           If x is not present, the largest step smaller than step_no is returned instead (i.e. a step preceding it)
        """
        try:
